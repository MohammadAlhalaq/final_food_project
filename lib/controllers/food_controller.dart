...
Future<CollectionReference> getSalad() async {
  FirebaseFirestore firestore = FirebaseFirestore.instance;
  CollectionReference salads = firestore.collection('saladModel');

  // Call the user's CollectionReference to add a new user
  return salads.get();
}

Future<List> CreateSaladWidget() async {
  var salad = await getSalad();
  List<SaladModel> salads = salad.docs.forEach((doc) => SaladModel(
      id: doc["id"],
      img: doc["img"],
      title: doc["title"],
...
class FoodController extends GetxController {
  List<SaladModel> salads = CreateSaladWidget() as List<SaladModel>;
  // [
  //   SaladModel(
  //     id: "1",
  ...
  Future<void> updateUser() {
    ...
    void addQ(int index) {
      if (salads[index].quantity < 9 && salads[index].quantity >= 0) {
        salads
            .doc(salads[index].id)
            .update({'quantity': salads[index].quantity++})
